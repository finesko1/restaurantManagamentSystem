...
trait AuthenticatesUsers
{
    ...
    public function login(Request $request)
    {
        ...
        // Определяем guard в зависимости от роли
        $guard = null;
        if ($role === 'waiter') {
            $guard = 'waiter';
        } elseif ($role === 'cook') {
        ...
        if (Auth::guard($guard)->attempt($request->only('email', 'password'))) {
            ...
            if ($user->role === $role) {
                ...
                Log::warning('Role mismatch', ['user' => $user, 'expected' => $role]);
                return redirect()->back()->withErrors(['role' => 'Role does not match.']);
            }
        } else {
        ...
        // Если попытка входа не удалась, увеличиваем количество попыток
        $this->incrementLoginAttempts($request);
        return $this->sendFailedLoginResponse($request);
    }
    ...
    {
        // Деавторизуем определенного охранника
